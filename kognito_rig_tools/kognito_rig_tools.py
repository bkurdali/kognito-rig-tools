...
class RigToggleHandFollow(bpy.types.Operator):
    ...
    @classmethod
    def poll(cls, context):
        return context.mode == 'POSE' and context.active_object.name == 'rig_ctrl'

    def execute(self, context):
        hand_ik_bones = [bone for bone in context.active_object.pose.bones if bone.name.startswith('forearm_ik')]
...
class RigToggleHandInheritRotation(bpy.types.Operator):
    ...
    def execute(self, context):
        hand_bones = [bone for bone in context.active_object.pose.bones if bone.name.startswith('hand.')]
